...
Root CA 私钥是整套信任链的核心。应用会优先把 Root CA 私钥写入系统密钥环；如果系统密钥环不可用，则回退到本地 AES-256-GCM 加密文件，并通过仅限当前用户读取的本地密钥保护。回退方案主要防止私钥明文落盘和意外泄露，不能抵御已经取得当前系统账户权限的攻击者。

Root CA 证书与私钥按材料 ID 分版本写入。只有新证书和私钥完成回读校验后，应用才切换活动材料，因此写入失败不会把现有 CA 留在证书、私钥不匹配的状态。旧版存储格式会继续兼容读取，并在下一次成功创建或导入后迁移。

重新初始化 Root CA 会替换当前 Root CA。客户端需要安装新的根证书；如果旧 Root CA 曾经导入系统信任区，应用不会自动执行高风险的信任删除操作，管理员应在确认旧证书不再使用后手动移除旧 Root CA。应用会要求二次确认，并输入固定确认文本后才允许进入重新初始化流程。
...
- 在 Unix 系统上，`server.key`、`server.pfx` 和 Root CA 备份包会以 `0600` 权限原子写入。
- 签发后的服务端私钥只保留在 Rust 进程内存，不会通过 Tauri IPC 返回前端页面。
- Electron 中忽略证书错误的方案仅限开发环境，不要在生产环境使用全局跳过证书校验。
- 本地回退加密不能替代操作系统账户安全、磁盘加密和可信终端管理。
...
发布标签必须与 `package.json`、`src-tauri/Cargo.toml` 和 `src-tauri/tauri.conf.json` 中的版本一致。发布流程会使用已提交的 npm/Cargo 锁文件，并为所有 ZIP 制品生成 `SHA256SUMS`。macOS 公证和 Windows Authenticode 签名仍需要项目维护者配置对应的开发者证书与 GitHub Secrets；没有签名凭据时产物应明确按未签名内部测试包分发。

## 项目文档

- 路线图：[RLOADMAP.md](./RLOADMAP.md)
- 发布签名配置：[RELEASE.md](./RELEASE.md)
